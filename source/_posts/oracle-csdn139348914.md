...
视图本身不存数据，只是一条封装好的查询。**敏感列不在视图的 SELECT 列表里，用户通过视图就永远碰不到**——这是列级隔离的标准做法，比在应用层过滤可靠得多。顺带一提，视图的 WHERE 子句也能做粗粒度的行级过滤（比如只暴露 `department_id = 10` 的行）；要按调用者身份动态控制的行级安全，Oracle 的对应机制是 VPD（`DBMS_RLS` 包），不在本文展开。
...
验证标准：视图查询成功、基表报 ORA-00942（用户根本"看不到"这张表）、写操作报 ORA-01031。三个都符合预期，只读隔离才算生效。
