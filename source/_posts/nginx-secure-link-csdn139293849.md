...
- **临时链接**：签名里带过期时间，超时自动失效，适合限时共享文件、下载地址防扩散。发出去的链接过期即 410，不需要后端再维护任何"撤销列表"——失效逻辑完全在 URL 自己身上，这是 secure_link 相比"数据库存 token"方案最省心的地方。顺带说一句 410 和 403 的语义分工：410 Gone 告诉客户端"曾经有效、现已失效"，对"拿着过期链接反复重试"的用户端是明确信号；403 则是"从头就不合法"。两者都拦得住，但前端据此引导用户的姿势完全不同——过期引导重新申请，403 该查的是链接分发环节。
...
- **时钟是这套机制的隐形前置条件**。`expires` 按服务器本地时间判定，下发签名的业务机和跑 Nginx 的机器时钟漂移几十秒，就会出现"刚下发的链接直接 410"这类灵异现象；两台机器都上 NTP 是上线前的固定动作。
- **密钥轮换要考虑已发链接**。换 `secret_key` 的瞬间，所有存量签名全部失效——已发出去的下载链接批量 403。平滑做法是轮换期同时保留新旧两个验证（两个 location 各配一个密钥，新链接发新签名，旧链接自然过期后再撤旧密钥）。
