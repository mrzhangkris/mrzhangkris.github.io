...
- 容器里可用的来源 IP 有两个：环回 `127.0.0.1` 和容器网卡 `172.17.0.3`，正好用来做"名单内/名单外"的对照。
...
实测用一个网段加两个 IP 验证边界：容器里以 `127.0.0.0/8` 复演网段放行，来源是环回 IP 的请求命中网段返回 200，来源是容器网卡 `172.17.0.3` 的请求落在网段外返回 403——同一台机器，两个来源，一放一拒：
