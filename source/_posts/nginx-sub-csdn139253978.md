...
sub_filter 只对显式配置了它的 location 生效，兄弟 location 不受影响。
...
默认 on 的输出里新旧域名并存——替换了第一处就收工。域名迁移场景几乎总是要 `off`，不然页面一半新链接一半旧链接，比不换还糊涂。
...
这就是"sub_filter 和压缩冲突"的完整链路。想两头兼得（替换后再压缩省带宽），可以同时在 Nginx 这一层开 gzip——对客户端的响应仍会被 Nginx 压缩，只是上游到 Nginx 这段走明文，内网链路的带宽代价通常可接受。
